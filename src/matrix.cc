...
void matrix22::eigenvalues(double& lambda0, double& lambda1) {
	...
	double c =   this->a*this->d - this->b*this->c;
	
	double a0 = -b / (2.0 * a);
	double a1 = sqrt(pow(b, 2.0) - 4 * a * c) / (2.0 * a);
	lambda0 = a0 + a1;
	lambda1 = a0 - a1;
}
...
void cMatrix::output() {
	for (int j = 0; j < m; j++) {
		for (int i = 0; i < n; i++) {
			std::cout << std::setw(16) << std::fixed << std::setprecision(10) << A[j * n + i] << "   ";
		}
		std::cout << std::endl;
	}
	std::cout << std::endl;
...
cMatrix::cMatrix() : m(2), n(2) {
	A = new double[m * n];
	this->eye();
}

cMatrix::cMatrix(const cMatrix& B) : m(0), n(0) {
	if (m * n != B.m * B.n) {
		//delete [] A;
...
cMatrix cMatrix::minor_(int i, int j) {
	cMatrix min(this->n-1, this->n-1);
	int _i, _j;
	_j = 0;
...
double cMatrix::det() {
	if (this->n == 2) {
		...
		for (int j = 0; j < this->n; j++) {
			det += (j % 2 ? -1 : 1) * this->A[j * this->n + 0] * this->minor_(0, j).det();
		}
		return det;
	}
}
...
cMatrix cMatrix::gaussJordanElimination(bool& singular) {
	cMatrix m = *this;
	cMatrix mInv = cMatrix(n, n);
	...
	for (int i = 0; i < n - 1; i++) { // column
		...
		for (int j = i + 1; j < n; j++) { // column
			if (fabs(m.A[j * n + pivot]) > fabs(m.A[i * n + pivot])) {
				double t;
				for (int l = 0; l < n; l++) {
					t = m.A[i * n + l];
					m.A[i * n + l] = m.A[j * n + l];
					m.A[j * n + l] = t;
					t = mInv.A[i * n + l];
					mInv.A[i * n + l] = mInv.A[j * n + l];
					mInv.A[j * n + l] = t;
				}
			}
		}
		for (int j = i + 1; j < n; j++) { // column
			double s = m.A[j * n + pivot]/m.A[i * n + pivot];
			for (int k = 0; k < n; k++) { // row
				m.A[j * n + k] -= s * m.A[i * n + k];
				mInv.A[j * n + k] -= s * mInv.A[i * n + k];
			}
		}
	}
